...
- **Fabric loader updating** — `Core/FabricLoaderUpdate.cs`, `UI/LoaderUpdateWindow`.
  ...
  - **Installing replaces.** The old profile and the libraries that then belong to
    nothing are deleted, so exactly one loader remains. This was not cosmetic: both
    `ClientLauncher` and `LoaderVersions` used to take the *first* `fabric-loader*.json`
    the filesystem listed, which sorts `0.19.2` above `0.19.3` — so the game could keep
    starting on the old loader while the launcher reported the new one. Both now ask
    `FabricLoaderUpdate.NewestProfile`.
  - A library is deleted only when **no remaining profile names it**, because that
    folder also holds the vanilla game's libraries and a Forge or NeoForge install's,
    and most of a Fabric profile's list is shared with the loader it replaces.
    Deleting by folder name would take the game with it.
  - **Nothing is removed until the replacement is on disk.** Asked to keep a version
    that is not installed, it removes nothing — a half-failed download must not leave
    the version with no loader at all.
...
zip: **1.2.266.222** (the zip is kept outside the repo — see section 8).
...
1.2.266.222 every other machine installs it on its own within a couple of minutes of
...
0. **Put 1.2.266.222 on the host.** Everything below matters less than this: it is
